"""run_leapfrog_deer.py

Benchmark: Newton iterations to DEER convergence vs leapfrog trajectory length.
...
from src.deer import seq1d as deer_seq1d
...
def newton_max_iter(num_leapfrog_steps: int) -> int:
    return int(num_leapfrog_steps) + 2


def leapfrog_step(state: jnp.ndarray, _driver, params) -> jnp.ndarray:
    eps = params["epsilon"]
    position, momentum = jnp.split(state, 2)
    position = position + eps * momentum
    grad = jax.grad(params["logp"])(position)
    momentum = momentum + eps * grad
...
def run_once(
    *,
    ...
    y0 = jnp.concatenate(
        [position0, momentum0 + 0.5 * epsilon * jax.grad(logp)(position0)]
    )
    params = {"epsilon": epsilon, "logp": logp}
    init_guess = jnp.broadcast_to(y0, (num_steps, y0.shape[0]))
    xinput = jnp.zeros((num_steps,), dtype=y0.dtype)

    _, conv_iter = deer_seq1d(
        leapfrog_step,
        y0,
        xinput,
        params,
...
def make_plot(sweep: list[tuple[int, int, bool]], *, target: str, epsilon: float, out_dir: Path):
    import matplotlib
    ...
    ax.set_xlabel("leapfrog steps L")
    ax.set_ylabel("Newton iterations to convergence")
    ax.set_title(f"{target}, ε={epsilon}")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()

    save_path = out_dir / f"leapfrog_deer_{target}_eps{epsilon}.png"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sweep-max", type=int, default=None, help="Max L in sweep (default: config or 512)")
    parser.add_argument("--epsilon", type=float, default=0.1)
    parser.add_argument("--random-seed", type=int, default=42)
    parser.add_argument("--initial-position-scale", type=float, default=1.0)
    parser.add_argument("--initial-momentum-scale", type=float, default=1.0)
...
def main():
    ...
    mom_scale = float(cfg.get("initial_momentum_scale", args.initial_momentum_scale))
    ...
    key = jr.PRNGKey(seed)
    key, kpos, kmom = jr.split(key, 3)
    position0 = pos_scale * jr.normal(kpos, (target.dim,))
    momentum0 = mom_scale * jr.normal(kmom, (target.dim,))
    ...
    print("=" * 72)
    print("Leapfrog DEER: Newton iterations vs trajectory length")
    print(
        f"  target={target.name}  D={target.dim}  ε={epsilon}  "
    ...
    if not args.no_plot:
        make_plot(sweep, target=target.name, epsilon=epsilon, out_dir=args.runs_dir.resolve())

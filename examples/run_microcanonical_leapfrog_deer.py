"""Run DEER on deterministic isokinetic leapfrog (microcanonical / MCLMC dynamics).

One isokinetic-Mclachlan step per chain transition, no momentum refresh. Parallel
trajectory solved with DEER; outputs progress plots and GIF.

Run:
    python examples/run_microcanonical_leapfrog_deer.py
    python examples/run_microcanonical_leapfrog_deer.py --target banana
"""
...
DEFAULT_SAMPLER_CONFIG = SAMPLER_CONFIGS_DIR / "microcanonical_leapfrog.json"
...
if D < 2:
    raise ValueError(
        f"Microcanonical dynamics requires dimension >= 2; target {args.target} has D={D}."
    )
...
print("Running parallel microcanonical leapfrog with full trace for visualization")
...
if __name__ == "__main__":
    ...
    hmc_plot.progress_plot(
        ...
        suptitle=f"{chain_length} microcanonical leapfrog draws (quasi={quasi})",
    )
    hmc_plot.newton_max_error_plot(
        states_par_np,
        rtol=rtol,
        savepath=plot_newton,
        title=f"DEER Newton error ({target.name}, microcanonical leapfrog)",
    )
    hmc_plot.newton_truth_error_plot(
        ...
        title=f"Parallel-vs-sequential trajectory error ({target.name}, microcanonical leapfrog)",
    )
